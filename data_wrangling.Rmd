...
```{r}
text_string <- pdf_text(path_anfragen_18_clean[77]) %>% 
     strsplit("\n") %>% 
     unlist()
partei<-text_string[4:7]
O <- str_detect(partei, "GRÜNE")

if(TRUE %in% O){
  parte <- "Grüne"
}
a<-grep("DIE LINKE", partei)
            if(length(a)>=1){
              qs<-"Linke"
            }

# 4 bis 7 kommt Partei vor
# creating vector for partei
party <- c()
for (i in path_anfragen_18_clean) {
  text <- pdf_text(path_anfragen_18_clean[i]) 
  text_string <- strsplit(text, "\n") %>% 
    unlist()
  #Bereich, indem Parteinamen steht
  partei<-text_string[4:7]
  a<-str_detect(partei, "SPD")
  if(TRUE %in% a){
    part<-"SPD"
  }
  
    else{
          b<-str_detect(partei, "DIE LINKE")
          if(TRUE %in% b){
            part<-"Linke"
          }
         ...
         c<-str_detect(partei, "CDU/CSU")
         if(TRUE %in% c){
           part<-"Union"
         }
          
          else{
          d<-str_detect(partei, "BÜNDNIS 90/DIE GRÜNEN")
          if(TRUE %in% d){
          part<-"Grüne"
          }
...
df_18 %>% 
  ...
  geom_text(aes(label = n), 
            size = 4, 
            vjust = -0.5) # edding label position
