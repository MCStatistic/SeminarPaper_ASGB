...
# inspect it again, because the code would be much cleaner as the other one...
content <- c()
for (i in seq_along(path_18_clean_v2)){
  # getting the content as whole string
  complete_string <- pdf_text(path_18_clean_v2[i]) %>% 
    unlist() %>% 
    toString()
  
  content_r <- strsplit(complete_string, "LINKE | GRÜNE | SPD | CDU/CSU") %>% 
  unlist() %>% 
  .[2]
...
text_string <- pdf_text(path_anfragen_18_clean[i]) %>% 
  ...
  if(TRUE %in% a){
    ...
    content_r <- strsplits(complete_string, "GRÜNEN") %>% 
       unlist() %>% 
      .[2]
  } else{
      b <- str_detect(partei, "LINKE")
      ...
      if(TRUE %in% b){
        complete_string <- pdf_text(path_18_clean_v2[i]) %>% 
         ...
         content_r <- strsplits(complete_string, "LINKE") %>% 
           unlist() %>% 
           .[2]
      } else{
        c <- str_detect(partei, "SPD")
        ...
        if(TRUE %in% c){
          ...
          content_r <- strsplits(complete_string, "SPD") %>% 
            unlist() %>% 
            .[2]
        } else{
          d <- str_detect(partei, "CDU/CSU")
          ...
          if(TRUE %in% d){
            ...
            content_r <- strsplits(complete_string, "CDU/CSU") %>% 
               unlist() %>% 
               .[2]
          } else{
          z <- "NA"
...
save(df_18_v2, file = "df_18_v3.RData")
load("df_18_v3.RData")



testpath <- path_18_clean_v2[1:100]
...
complete_string <- pdf_text(testpath[1]) %>% 
    unlist() %>% 
    toString() %>% 
    str_split(., "LINKE|GRÜNEN|SPD|CSU") %>% 
    unlist() %>% 
    .[2] %>% 
    str_replace(".", "")

# testing
# complete_string <- pdf_text(path_18_clean_v2[33]) %>% 
#     unlist() %>% 
#     toString() 
# 
# strsplit(complete_string, "GRÜNE | LINKE") %>% 
#   unlist() %>% 
#   .[2]
