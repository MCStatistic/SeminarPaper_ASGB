...
# linear
svm_v2 <- train_model(container_v2,"SVM", kernel = "linear")
svm_classify_v2 <- classify_model(container_v2, svm_v2)
...
length(which(test_svm_prob_v2$thema==test_svm_prob_v2$thema_test))/length(test_svm_prob_v2$thema)
nrow(test_svm_prob_v2)/nrow(test_svm_v2)
# 0.3: 0.8062827 richtig / 0.7670683 Matching
# 0.4: 0.8548 richtig / 0.6452 Matching
# 0.5: 0.8807 richtig / 0.5274 Matching
# 0.6: 0.8947 richtig / 0.4578 Matching
# 0.7: 0.9115 richtig / 0.4090 Matching
# 0.8: 0.9295 richtig / 0.3226 Matching
# 0.9: 0.9647 richtig / 0.2276 Matching
...
algo_compare %>% 
  ...
  geom_hline(yintercept = 0.9, color = "red", alpha = 0.3) +
  geom_hline(yintercept = 0.95, color = "green", alpha = 0.3) +
  theme(legend.position="bottom", legend.title = element_blank()) +
  facet_wrap(~ prob)

```

# plotting probabilities of svm and rf
```{r}
...
ggpreview(rf_svm_prob_boxplot, width = 6, height = 6.5)
ggsave("rf_svm_prob_boxplot.png", rf_svm_prob_boxplot, 
       path = "./images/", width = 6, height = 6.5)
```
