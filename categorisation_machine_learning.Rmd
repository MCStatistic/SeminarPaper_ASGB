...
test_rf_v2_200 <- df_training_v2[2301:3047,] %>% 
  select(-content, -bt, -party, -type, -bt_f, -title, -date)
test_rf_v2_200$thema_rf_200 <- rf_classify_v2$FORESTS_LABEL
test_rf_v2_200$prob_rf_200 <- rf_classify_v2$FORESTS_PROB
...
rf_500_plot_df <- rf_classify_v2_500 %>% 
...
prob_plot_df <- rbind.data.frame(rf_500_plot_df, svm_plot_df)
...
rf_svm_prob_boxplot <- prob_plot_df %>% 
  ...
  theme_bw() +
  xlab("") +
  ylab("Klassenwahrscheinlichkeit") +
  facet_wrap(~ model) +
...
prob_boxplot_real <- df_complete_clean_f %>% 
  ...
  filter(!is.na(thema_rf)) 

nrow(prob_boxplot_real)
...
prob_boxplot_real %<>% 
  ...
  mutate(model = case_when(
    ...
    model == "svm" ~ "Super Vector Machine"
    ))

prob_boxplot_real[1,]
prob_boxplot_real[3303,]
...
ggpreview(prob_boxplot_real_mod, width = 7, height = 6.5)
...
df_complete_clean_f %>%
  group_by(party, thema_rf) %>%
  filter(party != "SPD" & party != "Union" & party != "NA") %>% 
  summarise(n = n()) %>%
  mutate(perc = (n/sum(n))) %>%
  mutate(thema_rf = fct_reorder2(thema_rf, party, perc)) %>% 
  ggplot(aes(x = thema_rf, y = perc)) +
  geom_bar(position = "dodge",
           stat = "identity",
           fill = "skyblue4") +
