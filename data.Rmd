...
```{r}
urls_test <- urls[1:10]
destinations <- "/Users/marlonschumacher/Documents/2_Master/Git_Projekte"
for(i in seq_along(urls_test)){
    download.file(urls_test[i], destinations[i], mode="wb")
}
```
...
url_fun <- url_fun %>% 
  select(-port, -fragment, -parameter)

# splitting the url in 
url_fun$path_1 <- str_sub(url_fun$path, start = 1L, end = 14L)
url_fun$end <- str_sub(url_fun$path, start = -4L, end = -1L)
url_fun$first <- str_sub(url_fun$path, start = 15L, end = 18L)
...
length(first_v)
second_v <- as.character(sprintf("%04.0f", 0000:5799))
...
final.df <- as.data.frame(final_url_v, stringsAsFactors = F)
final.df <- final.df[(5200:5220),]
length(final.df)
```
