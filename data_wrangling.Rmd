...
plot_anfragen_18_19 <- df_complete_clean %>% 
  filter(party != "NA" & party != "SPD" & party != "Union") %>% 
  mutate(date_n = as.Date(date, "%d.%m.%Y")) %>% 
  mutate(year = year(date_n),
         party = case_when(party == "Union" ~ "CDU/CSU",
                           TRUE ~ party)) %>% 
...
ggpreview(plot_anfragen_18_19, width = 7.5, height = 8.5)
ggsave("AnfragenPartei_18_19.png", path = "./images/", width = 7.5, height = 8.5)
  

  
party <- c("Linke", "Grüne", "FDP", "CDU/CSU", "AfD", "SPD")
sitze18 <- c(70, 63, 0, 255, 0, 193)
sitze19 <- c(69, 67, 80, 246, 91, 152)
  
sitze <- cbind(party, sitze18, sitze19) %>% 
  as.data.frame() 
```
