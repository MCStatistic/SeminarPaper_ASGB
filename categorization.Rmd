...
### train model - testing two algorithm
```{r}
load("df_complete_clean_v2.RData")
...
prob_plot_df %>% 
  group_by(model) %>% 
  mutate(label = fct_reorder(label, prob)) %>% 
  ggplot(aes(x = label, y = prob, col = model)) +
...
ggpreview(rf_svm_prob_boxplot, width = 7, height = 6.5)
ggsave("rf_svm_prob_boxplot.png", rf_svm_prob_boxplot, 
       path = "./images/", width = 7, height = 6.5)
...
# SVM classification
svm_model <- train_model(container_full, "SVM", method = "linear")
...
# inspecting just a bit
df_complete_clean_f %>% 
  select(thema, party, file, thema_rf, prob_rf, thema_svm, prob_svm) %>% 
  filter(thema == "zSonstiges")
```
